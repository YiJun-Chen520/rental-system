...
<body>
    ...
    <div class="main-content">
        ...
        <div class="toolbar">
            <input type="text" id="keyword" placeholder="输入地址搜索" onkeydown="if(event.key==='Enter')loadData(1)">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody id="tableBody">
                    <tr>
                        <td colspan="9" style="text-align:center;color:#999;padding:40px;">加载中...</td>
                    </tr>
                </tbody>
            </table>
    ...
    <script>
        ...
        function loadData(page) {
            ...
            .catch(function(error) {
                ...
                document.getElementById('tableBody').innerHTML =
                    '<tr><td colspan="9" style="text-align:center;color:#ff4d4f;padding:40px;">加载失败，请稍后重试</td></tr>';
            });
        }
        ...
        function renderTable(list) {
            ...
            list.forEach(function(item) {
                ...
                html += '<td>' + (item.houseType || '') + '</td>';
                html += '<td>' + (item.area ? item.area + '㎡' : '') + '</td>';
                html += '<td>' + (item.rent ? '¥' + item.rent : '') + '</td>';
                html += '<td><span class="' + statusClass + '">' + statusText + '</span></td>';
                html += '<td>' + (item.ownerName || '') + '</td>';
                html += '<td>' + (item.createTime || '') + '</td>';
                html += '<td class="action-btns">';
                html += '<select class="status-select" onchange="changeStatus(' + item.id + ',this.value)" style="padding:3px 6px;border:1px solid #d9d9d9;border-radius:4px;font-size:12px;">';
                html += '<option value="0"' + (item.status === 0 ? ' selected' : '') + '>空闲</option>';
                html += '<option value="1"' + (item.status === 1 ? ' selected' : '') + '>已租</option>';
                html += '<option value="2"' + (item.status === 2 ? ' selected' : '') + '>下架</option>';
                html += '</select>';
                html += '<button class="btn btn-danger btn-small" onclick="deleteHouse(' + item.id + ')">删除</button>';
                html += '</td>';
                html += '</tr>';
            });
